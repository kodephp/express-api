 ...
 * 端点（以百世开放平台为准，此处为骨架）：
 *  - 下单：POST /open/api/order/create
 *  - 订单查询：GET /open/api/order/query/{orderId}
 *  - 轨迹查询：GET /open/api/trace/query/{trackingNumber}
 *  - 取消：POST /open/api/order/cancel/{orderId}
 *  - 面单：POST /open/api/label/print/{orderId}
 *  - 揽收通知：POST /open/api/pickup/create
 *  - 拦截件：POST /open/api/order/intercept/{orderId}
 *  - 改件：PUT /open/api/order/update/{orderId}
 */
class Client extends AbstractCourierClient
{
    ...
    /**
     * 构造百世签名请求头（app_key + timestamp + sign）
     *
    ...
    protected function prepareRequestHeaders(array $headers): array
    {
        ...
        $headers['X-Best-AppKey']   = $this->config->getAppKey();
        $headers['X-Best-Timestamp'] = $timestamp;
        $headers['X-Best-Sign']     = $sign;
        $headers['X-Best-Version']  = $this->config->getVersion();
    ...
    public function sendShipment(array $data): array
    {
        ...
        return $this->request('POST', '/open/api/order/create', $data);
    }
    ...
    public function queryOrder(string $orderId): array
    {
        ...
        return $this->request('GET', '/open/api/order/query/' . $orderId);
    }
    ...
    public function queryTracking(string $trackingNumber, string $language = 'zh-CN'): array
    {
        ...
        $uri = '/open/api/trace/query/' . $trackingNumber;
        if ($language) {
    ...
    public function cancelOrder(string $orderId, string $reason = ''): array
    {
        ...
        return $this->request('POST', '/open/api/order/cancel/' . $orderId, $data);
    }
    ...
    public function printLabel(string $orderId, array $data = []): array
    {
        ...
        return $this->request('POST', '/open/api/label/print/' . $orderId, $data);
    }
    ...
    public function pickupNotice(array $data): array
    {
        ...
        return $this->request('POST', '/open/api/pickup/create', $data);
    }
    ...
    public function intercept(string $orderId, array $data = []): array
    {
        ...
        return $this->request('POST', '/open/api/order/intercept/' . $orderId, $data);
    }
    ...
    public function modify(string $orderId, array $data): array
    {
        ...
        return $this->request('PUT', '/open/api/order/update/' . $orderId, $data);
    }
