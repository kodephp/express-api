...
class Client extends AbstractCourierClient
{
    ...
    /**
     * 构造圆通签名请求头（app_key + timestamp + sign）
     *
    ...
    protected function prepareRequestHeaders(array $headers): array
    {
        ...
        $headers['X-YTO-AppKey']   = $this->config->getAppKey();
        $headers['X-YTO-Timestamp'] = $timestamp;
        $headers['X-YTO-Sign']     = $sign;
        $headers['X-YTO-Version']  = $this->config->getVersion();
