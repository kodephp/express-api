...
/**
 * SingPost（新加坡邮政）API 客户端
 *
 * 真实接入：标准 REST，API key 鉴权（请求头 Authorization: Bearer <apiKey>）。
 * 端点（以 SingPost API 文档为准，此处为骨架）：
 *  - 轨迹：GET /pubtrack/v1/track/{trackingNumber}
 *  - 下单：POST /parcels/v1/parcels（海关申报随 parcels.customs 提交）
 *  - 报价：POST /pricing/v1/quote
 *  - 订单：GET /parcels/v1/parcels/{id}
 *  - 取消：DELETE /parcels/v1/parcels/{id}
 *  - 面单：GET /parcels/v1/parcels/{id}/label
 */
class Client extends AbstractInternationalClient
{
    ...
    /**
     * 注入 SingPost 专属请求头（API key 鉴权，以 Bearer 形式携带）
     */
    protected function prepareRequestHeaders(array $headers): array
    {
        $headers['Authorization'] = 'Bearer ' . $this->config->getAppKey();
    ...
    public function queryTracking(string $trackingNumber, string $language = 'en-US'): array
    {
        ...
        return $this->transmit(
            'GET',
            $this->config->getBaseUrl() . '/pubtrack/v1/track/' . $trackingNumber,
            [],
    ...
    public function sendShipment(array $data): array
    {
        ...
        return $this->transmit(
            'POST',
            $this->config->getBaseUrl() . '/parcels/v1/parcels',
            $payload,
            $this->prepareRequestHeaders([])
        );
    }
    ...
    public function getQuotation(array $data): array
    {
        ...
        return $this->transmit(
            'POST',
            $this->config->getBaseUrl() . '/pricing/v1/quote',
            $payload,
            $this->prepareRequestHeaders([])
        );
    }
    ...
    public function queryOrder(string $orderId): array
    {
        ...
        return $this->transmit(
            'GET',
            $this->config->getBaseUrl() . '/parcels/v1/parcels/' . $orderId,
            [],
            $this->prepareRequestHeaders([])
        );
    }
    ...
    public function printLabel(string $orderId, array $data = []): array
    {
        ...
        return $this->transmit(
            'GET',
            $this->config->getBaseUrl() . '/parcels/v1/parcels/' . $orderId . '/label',
            [],
            $this->prepareRequestHeaders([])
        );
    }
    ...
    public function declareCustoms(array $data): array
    {
        ...
        throw new ExpressApiException(
            'SingPost 海关申报随 createShipment 一并提交（parcels.customs），无需单独调用'
        );
    }
}
